...
export default {
	...
	options: [
		...
		{
			displayName: `<a target="_blank" href="https://open.feishu.cn/document/server-docs/im-v1/batch_message/read-users-for-batch-messages">${WORDING.OpenDocument}</a>`,
			name: 'notice',
			type: 'notice',
			default: '',
		},
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject> {
		...
		const { data } = await RequestUtils.request.call(this, {
			method: 'GET',
			url: `/open-apis/im/v1/batch_messages/${batch_message_id}/read_user`,
		});
