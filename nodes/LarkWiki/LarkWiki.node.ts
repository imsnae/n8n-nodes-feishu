...
function filterWikiProperties(allProps: any[]): any[] {
	...
	for (const prop of allProps) {
		...
		if (show.resource?.includes(WIKI_RESOURCE)) {
			filtered.push(prop);
		}
	}
	return filtered;
...
const wikiProperties = filterWikiProperties(resourceBuilder.build());

export class LarkWiki implements INodeType {
	description: INodeTypeDescription = {
		...
		version: [1],
		defaultVersion: 1,
		description: 'Lark Wiki Spaces Operations (Nodes, Members, Settings)',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const callFunc = resourceBuilder.getCall(WIKI_RESOURCE, operation);

		if (!callFunc) {
			throw new NodeOperationError(
				this.getNode(),
				'No operation found: ' + WIKI_RESOURCE + '.' + operation,
			);
		}

		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				this.logger.debug('call function', { resource: WIKI_RESOURCE, operation, itemIndex });
